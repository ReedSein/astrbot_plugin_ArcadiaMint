...
class ArcadiaMint(Star):
    ...
    async def _collect_images(self, event: AstrMessageEvent) -> list[str]:
        ...
        for comp in msg_obj.message:
            if isinstance(comp, Image):
                try: images.append(await comp.convert_to_base64())
                except: pass
        for comp in msg_obj.message:
            if isinstance(comp, Reply) and comp.chain:
                for node in comp.chain:
                    if isinstance(node, Image):
                        try: images.append(await node.convert_to_base64())
                        except: pass
        return images
    ...
    async def _check_permission(self, event: AstrMessageEvent) -> tuple[bool, str]:
        ...
        if whitelist and gid and str(gid) not in whitelist: return False, ""
        ...
        if gid and limit > 0:
            async with self.rate_limit_lock:
                ...
                if count >= limit: return False, "🚫 本群调用太频繁，请稍后再试。"
    ...
    async def _execute_generation(self, event, prompt, images):
        path, err = await ttp.generate_image(
            ...
            timeout_seconds=self.config.get("timeout", 120), # 读取配置中的超时
            input_images=images
        )
    ...
    async def add_key(self, event: AstrMessageEvent, key: str):
        ...
        if event.get_sender_id() not in admin_ids: return 
